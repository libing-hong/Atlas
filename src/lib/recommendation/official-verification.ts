...
export const plausibleInstitutionName = (value: string | null) => Boolean(value && value.length >= 4 && value.length <= 120 && !genericPageCopy.test(value) && !/^(?:world|top|best|leading|global)\s+(?:university|school|college)$/i.test(value.trim()) && /university|universit[eé]|school|college|école|institute|institut/i.test(value));
...
export async function verifyProgrammeLead(lead: ProgrammeLead, profile: UnderstoodProfile): Promise<{ programme?: VerifiedProgramme; rejection?: RejectedProgrammeLead }> {
  ...
  const institutionName = plausibleInstitutionName(claimed.institution) && supportedByPage(claimed.institution!, body, finalParsed)
    ? claimed.institution
    : plausibleInstitutionName(extractedInstitutionName) ? extractedInstitutionName : null;
  const programmeName = plausibleProgrammeName(claimed.programme) && supportedByPage(claimed.programme!, body, finalParsed)
    ? claimed.programme
    : plausibleProgrammeName(extractedProgrammeName) ? extractedProgrammeName : null;
