...
function recordPriority(record: ApplicationRecord) {
  ...
  if (!["conditional_offer", "unconditional_offer", "accepted", "submitted", "waiting_result"].includes(record.status) && daysUntil(record.nextDeadline) <= 30) return 1;
  if (record.status === "ready_to_submit") return 2;
  if (record.missingMaterials.length) return 4;
  if (record.status === "submitted" || record.status === "waiting_result") return 5;
  if (["conditional_offer", "unconditional_offer", "accepted"].includes(record.status)) return 6;
  return 7;
}
...
function taskCopy(record: ApplicationRecord) {
  ...
  if (record.status === "ready_to_submit") return {
  ...
  if (["conditional_offer", "unconditional_offer", "accepted"].includes(record.status)) return {
...
export function getJourneyStagesForApplicationRecords(records: ApplicationRecord[]): JourneyStage[] {
  const currentIndex = records.some((record) => record.isFinalOffer && record.offerEvidenceAvailable) ? 2 : records.some((record) => ["conditional_offer", "unconditional_offer", "accepted"].includes(record.status)) ? 1 : 0;
  ...
  return [...applicationRecords]
    ...
    .map((record) => {
      ...
      return {
        id: record.id,
        title: copy.title,
        stage: record.isFinalOffer && record.offerEvidenceAvailable ? "Visa" : ["conditional_offer", "unconditional_offer", "accepted"].includes(record.status) ? "Offer" : "Application",
        explanation: copy.explanation,
        status: record.status === "submitted" || record.status === "waiting_result" ? "in_progress" : ["conditional_offer", "unconditional_offer", "accepted"].includes(record.status) ? "awaiting_evidence" : record.status === "supplement_required" ? "blocked" : "ready",
        ...
        actionHref: record.isFinalOffer && record.offerEvidenceAvailable ? "/dashboard/visa" : `/applications/${encodeURIComponent(record.id)}/materials`,
