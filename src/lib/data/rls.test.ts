...
const sql = [
  "supabase/migrations/202607190001_production_identity_and_rls.sql",
  "supabase/migrations/202607240002_private_beta_accounts_and_state.sql",
].map((file) => readFileSync(resolve(file), "utf8")).join("\n");

test("all user-owned production tables enable RLS", () => {
  for (const table of ["users", "student_profiles", "advisor_assignments", "planning_runs", "recommendation_runs", "recommendation_candidates", "programme_verifications", "application_records", "materials", "service_orders", "consents", "privacy_requests", "audit_events", "atlas_user_states", "review_queue", "product_events"]) {
