...
export async function POST(request: Request, context: { params: Promise<{ programId: string }> }) {
  ...
  try {
    ...
    const body = await parseJson(request, ingestionRequestSchema, 4_000); const result = await enqueueProgramIngestion(programId, body.trigger ?? "manual_retry", body.priority ?? 80); return apiResponse(ctx, result, result.queued ? 202 : 503);
