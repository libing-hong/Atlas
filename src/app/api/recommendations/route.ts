...
import { recordProductEvent } from "@/lib/server/audit";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    await recordProductEvent({ eventName: "recommendation.completed", module: "recommendations", success: result.generationStatus !== "empty", durationMs: Date.now() - startedAt, metadata: { candidateCount: candidates.length, verifiedCount, generationStatus: result.generationStatus ?? "unknown" } });
    ...
    await recordProductEvent({ eventName: "recommendation.failed", module: "recommendations", success: false, durationMs: Date.now() - startedAt, metadata: { errorCode: code ?? "unknown" } });
