...
export default function ResultPage() {
  ...
  return <main className="atlas-shell py-8"><BackHome /><section className="grid gap-6 lg:grid-cols-[0.72fr_1.28fr]">
    ...
    <div className="space-y-6"><Card><CardHeader title="推荐国家匹配度" /><div className="space-y-5">{report.countryFit.map((item) => <div key={item.country}><div className="mb-2 flex items-center justify-between text-sm"><span className="font-medium">{item.country}</span><span className="text-[#8f847a]">{item.fit}%</span></div><ProgressBar value={item.fit} /><p className="mt-2 text-sm text-[#6f6256]">{item.note}</p></div>)}</div></Card>
      ...
      <Card><CardHeader title="主要推荐项目" />{recommendationStatus === "loading" ? <div className="flex items-center gap-3 rounded-2xl bg-[#f7f0e8] p-5 text-sm text-[#6f6256]"><LoaderCircle className="animate-spin" size={18} />Atlas 正在根据你的真实背景生成选校方案…</div> : programmeCandidates.length ? <div className="grid gap-3 md:grid-cols-3">{programmeCandidates.slice(0, 6).map((candidate) => <div key={candidate.officialProgrammeUrl} className="rounded-2xl border border-[#e8dfd3] bg-[#f7f0e8] p-4"><p className="font-medium">{candidate.institutionName}</p><p className="mt-1 text-sm text-[#6f6256]">{candidate.programmeName}</p><p className="mt-3 text-xs text-[#8f847a]">方案匹配度 {candidate.score} · {candidate.verificationStatus !== "verified" ? "待 Atlas 核验" : "已核验"}</p></div>)}</div> : recommendationStatus === "error" ? <div className="rounded-2xl bg-[#f6e7df] p-5 text-sm leading-6 text-[#8a5f54]">推荐生成失败，请稍后重试。</div> : <div className="rounded-2xl bg-[#fbf2df] p-5 text-sm leading-6 text-[#7b6541]">Atlas 暂未生成可用的学校推荐，请重新尝试。</div>}</Card>
