...
export function SensitiveUploadPanel() {
  ...
  return (
    <section className="rounded-[22px] border-2 border-[#d5c2ad] bg-[#fffaf3] p-5 md:p-6">
      ...
      {!uploadsEnabled ? <p className="mt-5 rounded-xl bg-[#fbf2df] p-3 text-sm text-[#7b6541]">Demo Mode：真实敏感文件上传尚未开放，请勿选择护照、签证、成绩单或资金证明。</p> : null}
      <label className="mt-5 flex items-center gap-3 rounded-xl border border-[#d8ccbe] bg-[#fbf6ef] p-3 text-sm text-[#4a3d34]">
        <input type="checkbox" disabled={!uploadsEnabled} checked={consented} onChange={(event) => setConsented(event.target.checked)} className="h-4 w-4 accent-[#2f2924]" />
        <span>我已了解 Atlas 将识别该文件中的身份及申请信息，仅用于生成材料清单、核验任务状态和提供办理辅助。</span>
      </label>
      <div className="mt-4 flex flex-wrap items-center gap-3">
        <label className={uploadsEnabled && consented ? "inline-flex cursor-pointer items-center gap-2 rounded-full bg-[#2f2924] px-4 py-2.5 text-sm text-[#fffaf3]" : "inline-flex cursor-not-allowed items-center gap-2 rounded-full bg-[#d8ccbe] px-4 py-2.5 text-sm text-[#8f847a]"}>
          <FileUp size={16} /> 选择文件
          <input type="file" accept="image/*,.pdf,.eml,.msg,.txt" disabled={!uploadsEnabled || !consented} className="sr-only" onChange={(event) => { const file = event.target.files?.[0]; if (file) { setFileName(file.name); setMessage("授权记录已生成，文件仅会在正式私有存储启用后保存。"); } }} />
        </label>
