...
export function SensitiveUploadPanel() {
  ...
  function selectFile(file?: File) {
    ...
    setMessage("本地模拟已记录文件名称和状态；文件内容没有上传到 Atlas 服务器。");
  }
    ...
    <section className="rounded-[22px] border-2 border-[#d5c2ad] bg-[#fffaf3] p-5 md:p-6">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3">
        <label className={consented ? "inline-flex cursor-pointer items-center gap-2 rounded-full bg-[#2f2924] px-4 py-2.5 text-sm text-[#fffaf3]" : "inline-flex cursor-not-allowed items-center gap-2 rounded-full bg-[#d8ccbe] px-4 py-2.5 text-sm text-[#8f847a]"}>
          <FileUp size={16} /> 选择文件
          <input type="file" accept="image/*,.pdf,.eml,.msg,.txt,.doc,.docx" disabled={!consented} className="sr-only" onChange={(event) => { selectFile(event.target.files?.[0]); event.target.value = ""; }} />
        </label>
        <button type="button" onClick={() => { setFileName(""); window.localStorage.removeItem("atlas.prototype.material-selection.v1"); setMessage("当前模拟文件记录已删除。"); }} className="rounded-full border border-[#d8ccbe] px-4 py-2.5 text-sm text-[#4a3d34]">删除文件</button>
