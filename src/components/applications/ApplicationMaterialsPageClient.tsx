...
export function ApplicationMaterialsPageClient({ applicationId }: { applicationId: string }) {
  ...
  const cachedSchool = candidate ? programmeCandidateToSchoolRecommendation(candidate) : undefined;
  const school: SchoolRecommendation = {
    ...
    deadlineType: cachedSchool?.deadlineType ?? "official",
    category: cachedSchool?.category ?? "manual_review",
    reasons: cachedSchool?.reasons ?? [],
    matchedRequirements: [],
    risks: cachedSchool?.risks ?? [],
    requirements: cachedSchool?.requirements ?? commonMaterials.map((item) => item.name),
    materialsReady: record.detectedMaterialCount,
    materialsTotal: Math.max(record.totalMaterials, commonMaterials.length),
    isSelected: true,
    isConfirmed: true,
    officialProgramUrl: cachedSchool?.officialProgramUrl ?? record.officialProgramUrl,
    applicationUrl: cachedSchool?.applicationUrl ?? record.applicationPortalUrl,
    applicationProvider: cachedSchool?.applicationProvider,
    applicationLinkStatus: cachedSchool?.applicationLinkStatus ?? record.applicationLinkStatus ?? "needs_review",
    admissionRequirements: record.programmeEvidenceSnapshot?.admissionRequirements ?? cachedSchool?.admissionRequirements,
    recommendationContent: {
      summary: cachedSchool?.recommendationContent.summary ?? "Atlas 已根据本次规划创建材料工作区。",
      personalFit: cachedSchool?.recommendationContent.personalFit ?? "",
      schoolHighlights: cachedSchool?.recommendationContent.schoolHighlights ?? "",
      programHighlights: cachedSchool?.recommendationContent.programHighlights ?? "",
      cautions: cachedSchool?.recommendationContent.cautions ?? [],
      sources: record.programmeEvidenceSnapshot?.sources ?? cachedSchool?.recommendationContent.sources ?? [],
    },
  };
