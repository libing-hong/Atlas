...
export function MaterialsWorkspaceClient({ school, applicationId }: { school: SchoolRecommendation; applicationId: string }) {
  ...
  const pending = useMemo(() => requirements.filter((item) => item.status === "needs_confirmation" && !effectiveConfirmedIds.includes(item.id)), [effectiveConfirmedIds, requirements]);
  ...
  const unreadyMaterials = useMemo(() => baseMaterials.filter((material) => !["prepared", "confirmed"].includes(statuses[material.id] ?? material.status)), [baseMaterials, statuses]);
  const allMaterialsReady = baseMaterials.length > 0 && unreadyMaterials.length === 0;
  const applicationReady = allMaterialsReady && pending.length === 0;
  useEffect(() => {
    const detectedMaterialCount = baseMaterials.length - unreadyMaterials.length;
    updateApplicationRecord(applicationId, {
      detectedMaterialCount,
      preparedMaterials: detectedMaterialCount,
  ...
  return <div className="space-y-6">
    ...
    <Card><div className="flex items-end justify-between gap-4"><div><p className="text-xs uppercase tracking-[0.22em] text-[#9a8b7c]">材料状态</p><h2 className="mt-2 font-editorial text-3xl font-semibold text-[#2f2924]">申请材料</h2><p className="mt-2 text-sm leading-6 text-[#6f6256]">Atlas 会将上传的通用材料同步到材料中心，并可用于其他学校。</p></div><span className="text-sm text-[#8f847a]">已确认 {baseMaterials.length - unreadyMaterials.length}/{baseMaterials.length} 项</span></div><div className="mt-5 grid gap-3 md:grid-cols-2">{baseMaterials.map((material) => <MaterialRow key={material.id} material={material} status={statuses[material.id] ?? material.status} fileName={files[material.id]} reusable={storedMaterials.filter((item) => !materialHint(material.name) || item.kind === materialHint(material.name))} onReuse={(id) => reuseMaterial(material, id)} onUpload={() => openPicker(material.id)} onConfirm={() => confirmFile(material)} onPreview={() => preview(material)} />)}</div><div className="mt-6 border-t border-[#e8dfd3] pt-5">{applicationReady ? <><div className="flex flex-col justify-between gap-4 rounded-2xl border border-[#bfd2bb] bg-[#f0f5ef] p-4 sm:flex-row sm:items-center"><div><p className="font-medium text-[#36573c]">材料和关键信息已齐</p><p className="mt-1 text-sm text-[#58705b]">下一步选择自行申请或 Atlas 代递交服务。</p></div><button type="button" onClick={() => setSubmissionOpen((value) => !value)} className={greenButton}>选择申请方式<ArrowUpRight size={16} /></button></div>{submissionOpen ? <SubmissionChoices onChoose={chooseService} /> : null}</> : <div className="rounded-2xl border border-[#ead8b6] bg-[#fbf3df] p-4"><p className="font-medium text-[#765d31]">完成以下事项后即可申请</p>{unreadyMaterials.length ? <p className="mt-2 text-sm text-[#806b46]">待补充材料：{unreadyMaterials.map((material) => material.name).join("、")}</p> : null}{pending.length ? <p className="mt-2 text-sm text-[#806b46]">待确认要求：{pending.map((item) => item.label).join("、")}</p> : null}</div>}</div></Card>
