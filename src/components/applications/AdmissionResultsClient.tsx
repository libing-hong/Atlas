...
import { CheckCircle2, Clock3, FileUp, PlaneTakeoff } from "lucide-react";
...
export function AdmissionResultsClient({ records }: { records: ApplicationRecord[] }) {
  ...
  const hasOfferEvidence = items.some((item) => item.offerEvidenceAvailable);
  ...
  async function uploadOffer(event: ChangeEvent<HTMLInputElement>) {
    ...
    updateApplicationRecord(target.id, { status: "conditional_offer", decisionStatus: "offer_received", offerSource: "student", offerEvidenceAvailable: true, offerFileName: file.name, applicationProgress: 95, nextAction: "确认 Offer 条件与最终入读选择" });
    setItems((current) => current.map((item) => item.id === target.id ? { ...item, status: "conditional_offer", decisionStatus: "offer_received", offerSource: "student", offerEvidenceAvailable: true, offerFileName: file.name, applicationProgress: 95, nextAction: "确认 Offer 条件与最终入读选择" } : item));
    event.target.value = "";
  }
