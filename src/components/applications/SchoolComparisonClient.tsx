...
function comparisonRows(profile: StudentProfile): Array<{ label: string; value: (school: SchoolRecommendation) => React.ReactNode }> {
  return [
    { label: "学校名称", value: (school) => school.universityName },
    { label: "专业名称", value: (school) => school.programName },
    { label: "当前匹配结果", value: (school) => publicMatchStatus(school, profile) },
    { label: "Atlas 方案匹配度", value: (school) => `${calculateProgramMatch(profile, school).matchScore} / 100（用于方案排序，不代表录取概率）` },
    ...
    { label: "课程与专业方向", value: (school) => getProgramContent(school.id)?.learningFocus.slice(0, 4).join("、") || "Atlas 正在核实该专业的最新官方信息。" },
    ...
    { label: "当前风险或待确认事项", value: (school) => calculateProgramMatch(profile, school).unresolvedItems.join("；") || school.risks.join("；") || "需要进一步确认" },
    { label: "Atlas 建议的下一步", value: (school) => calculateProgramMatch(profile, school).preparationItems.map((item) => item.label).join("、") || "确认申请轮次与截止日期后加入申请计划" },
  ];
}

function publicMatchStatus(school: SchoolRecommendation, profile: StudentProfile) {
  const result = calculateProgramMatch(profile, school);
  if (result.eligibilityStatus === "currently_not_eligible") return "存在明显差距";
  if (result.eligibilityStatus === "needs_confirmation") return "需要进一步确认";
  if (result.eligibilityStatus === "mostly_eligible") return "基本符合，仍需补充材料";
  return "符合公开要求";
}
...
function ComparisonStatus({ school, profile }: { school: SchoolRecommendation; profile: StudentProfile }) {
  const label = publicMatchStatus(school, profile);
